...
describe('StringCalculator', () => {
  ...
  describe('Add', () => {
    ...
    describe('Product of numbers when delimiter is "*"', () => {
      it('should give 10 for input [1,2,5]', () => {
        const product = calculator.add('//[*]\n1*2*5');
        expect(product).toBe(10);
      });

      it('should give 63840 for input [1,2,5,6,7,8,19]', () => {
        const product = calculator.add('//[*]\n1*2*5*6*7*8*19');
        expect(product).toBe(63840);
      });
    });
  });
});
